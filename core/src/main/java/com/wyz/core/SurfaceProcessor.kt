...
class SurfaceProcessor {
    ...
    private var mRenderer: WrapRenderer? = null
    ...
    fun setRenderer(renderer: Renderer) {
        mRenderer = WrapRenderer(renderer)
    }
    ...
    private fun glRun() {
        val iTextureProvider = mProvider ?: return
        val egl = EGLHelper()
        ...
        if (mRenderer == null) {
            mRenderer = WrapRenderer(null)
        }
        mRenderer?.create()
        mRenderer?.sizeChanged(mSourceWidth, mSourceHeight)
        mRenderer?.setFlag(if (iTextureProvider.isLandscape()) WrapRenderer.TYPE_CAMERA else WrapRenderer.TYPE_MOVE)
        ...
        while (!iTextureProvider.frame() && mGLThreadFlag) {
            ...
            mInputSurfaceTexture.getTransformMatrix(mRenderer?.textureMatrix)
