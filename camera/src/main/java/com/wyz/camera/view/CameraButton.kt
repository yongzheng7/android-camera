...
import com.wyz.camera.R
import com.wyz.camera.utils.PermissionUtils
...
class CameraButton : View {
    ...
    constructor(context: Context, attrs: AttributeSet?) : super(context, attrs) {
        ...
        button_size = tArray.getDimensionPixelSize(R.styleable.CircleImageView_Diameter, 100)
        tArray.recycle()
    ...
    private fun handlerUnpressByState() {
        ...
        when (state) {
            STATE_PRESS -> if (captureLisenter != null && (button_state == BUTTON_STATE_ONLY_CAPTURE || button_state ==
                            BUTTON_STATE_BOTH)) {
    ...
    private fun recordEnd() {
        if (captureLisenter != null) {
            if (recorded_time < min_duration) captureLisenter?.recordShort(recorded_time.toLong()) //回调录制时间过短
            else captureLisenter?.recordEnd(recorded_time.toLong()) //回调录制结束
        }
    ...
    inner class RecordCountDownTimer : CountDownTimer {
    ...
    inner class LongPressRunnable : Runnable {
        override fun run() {
            ...
            //没有录制权限
            if (PermissionUtils.getRecordState() != PermissionUtils.STATE_SUCCESS) {
                state = STATE_IDLE
                if (captureLisenter != null) {
    ...
    //设置回调接口
    fun setCaptureLisenter(captureLisenter: CameraListener) {
